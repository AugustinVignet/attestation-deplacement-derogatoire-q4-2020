...
app.get('/gus', (req, res) => {
    res.status(302).redirect(`/index?data=${encodeURIComponent(JSON.stringify(gus))}&type=${req.query.type}&express=${req.query.express}`)
});

app.get('/flo', (req, res) => {
    res.status(302).redirect(`/index?data=${encodeURIComponent(JSON.stringify(flo))}&type=${req.query.type}&express=${req.query.express}`)
});
